...
if __name__ == '__main__':
    ...
    easy_apply_config = config['easy_apply_bot']
    browser_config = config['browser']

    # Validate required parameters
    assert len(easy_apply_config['positions']) > 0
    assert len(easy_apply_config['locations']) > 0
    assert linkedin_config['username'] is not None
    assert linkedin_config['password'] is not None
    assert easy_apply_config['phone_number'] is not None

    # Create bot instance with new config structure
    bot = EasyApplyBot(
        linkedin_config['username'],
        linkedin_config['password'],
        easy_apply_config['phone_number'],
        easy_apply_config['salary'],
        easy_apply_config['rate'],
        uploads=easy_apply_config.get('uploads', {}),
        filename=easy_apply_config.get('output_filename', 'output.csv'),
        blacklist=easy_apply_config.get('blacklist', []),
        blackListTitles=easy_apply_config.get('blackListTitles', []),
        experience_level=easy_apply_config.get('experience_level', []),
        gemini_api_key=linkedin_config.get('gemini_api_key')
    )
    bot.start_apply(easy_apply_config['positions'], easy_apply_config['locations'])